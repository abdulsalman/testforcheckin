...
namespace ArabWaha
{
	public static class BatchRequest
	{
		...
		{
			StringBuilder sbBatchHeader = new StringBuilder();
			// "IndividualApplicationDetailsSet?$filter=applicationID%20eq%20{0}%20and%20locale%20eq%20'{1}'"
			var individualApplicationFilter =
				String.Format("GET IndividualApplicationDetailsSet?$filter=applicationID%20eq%20{0}%20and%20locale%20eq%20'{1}'&$format=json HTTP/1.1",
							  applicationId,
							  DebugDataSingleton.Instance.Language);

			var appliedJobDetailNoteFilter =
				String.Format("GET AppliedJobDetailNotesSet?$filter=applicationId%20eq%20'{0}'%20and%20locale%20eq%20'{1}'&$format=json HTTP/1.1",
							  applicationId,
							  DebugDataSingleton.Instance.Language);

			var coverLetterByIdSetFilter =
				String.Format("GET getCoverLetterByIdSet?$filter=CoverLetterId%20eq%20'{0}'&$format=json HTTP/1.1",
							  coverLetterId);
			...
			foreach (var subitem in apiName)
			{
				...
				sbBatchHeader.AppendLine(string.Format("Authorization: {0}", DebugDataSingleton.Instance.BasicAuth));
				sbBatchHeader.AppendLine("\r");
				sbBatchHeader.AppendLine("\r");
			}
		...
		public static string GenerateAppliedJobListBatchHeader(List<string> jobPostId)
		{
			StringBuilder sbBatchHeader = new StringBuilder();

			foreach (var subitem in jobPostId)
			{
				sbBatchHeader.AppendLine("--batch");
				sbBatchHeader.AppendLine("Content-Type:application/http");
				sbBatchHeader.AppendLine("Content-Transfer-Encoding: binary");
				...
				// pass jobPostId instead of 4568449 on navigating from joblist to details
				sbBatchHeader.AppendLine(string.Format("GET getJobDetailsSet?$filter=jobPostId%20eq%20'{0}'%20and%20language%20eq%20'{1}'&$format=json HTTP/1.1", subitem, DebugDataSingleton.Instance.Language));
		...
		public static string ForProfile()
		{
			...
			sbRequest.AppendLine("GET GetIndividualProfileDetailsSet?$format=json HTTP/1.1");
			sbRequest.AppendLine("\r");
			sbRequest.AppendLine("\r");
			sbRequest.AppendLine("--batch");
			sbRequest.AppendLine("Content-Type:application/http");
			...
			sbRequest.AppendLine("GET EducationSet?$format=json HTTP/1.1");
           // sbRequest.AppendLine("APIAuthorization:" + DebugDataSingleton.Instance.BasicAuth);
			sbRequest.AppendLine("\r");
			sbRequest.AppendLine("\r");
			sbRequest.AppendLine("--batch");
			sbRequest.AppendLine("Content-Type:application/http");
			...
			sbRequest.AppendLine("GET LicenseSet?$format=json HTTP/1.1");
            //sbRequest.AppendLine("APIAuthorization:" + DebugDataSingleton.Instance.BasicAuth);
			sbRequest.AppendLine("\r");
			sbRequest.AppendLine("\r");
			sbRequest.AppendLine("--batch");
			...
			sbRequest.AppendLine("GET ProfessionalExperienceSet?$format=json HTTP/1.1");
            //sbRequest.AppendLine("APIAuthorization:" + DebugDataSingleton.Instance.BasicAuth);
			sbRequest.AppendLine("\r");
			sbRequest.AppendLine("\r");
			sbRequest.AppendLine("--batch");
			...
			sbRequest.AppendLine("GET ReferenceSet?$format=json HTTP/1.1");
            //sbRequest.AppendLine("APIAuthorization:" + DebugDataSingleton.Instance.BasicAuth);
			sbRequest.AppendLine("\r");
			sbRequest.AppendLine("\r");
			sbRequest.AppendLine("--batch");
			...
			sbRequest.AppendLine("GET CompetencySet?$format=json HTTP/1.1");
            //sbRequest.AppendLine("APIAuthorization:" + DebugDataSingleton.Instance.BasicAuth);
			sbRequest.AppendLine("\r");
			sbRequest.AppendLine("\r");
			sbRequest.AppendLine("--batch");
			...
			sbRequest.AppendLine("GET TrainingandCertificateSet?$format=json HTTP/1.1");
            //sbRequest.AppendLine("APIAuthorization:" + DebugDataSingleton.Instance.BasicAuth);
			sbRequest.AppendLine("\r");
			sbRequest.AppendLine("\r");
			sbRequest.AppendLine("--batch--");
		...
		public static string ForObligations(string locale, string date)
		{
			string batchRequest = string.Empty;
			...
			sbRequest.AppendLine("Content-Transfer-Encoding: binary");
			sbRequest.AppendLine("\r");
			//sbRequest.AppendLine("GET getAnnouncementDetailsSet?$filter=individualId%20eq%20'" + userID + "'&$format=json HTTP/1.1");
			sbRequest.AppendLine("GET getAnnouncementDetailsSet?$format=json HTTP/1.1");
			sbRequest.AppendLine("\r");
			sbRequest.AppendLine("\r");
			sbRequest.AppendLine("--batch");
			sbRequest.AppendLine("Content-Type:application/http");
			sbRequest.AppendLine("Content-Transfer-Encoding: binary");
			sbRequest.AppendLine("\r");
			//sbRequest.AppendLine("GET getProgramObligationsSet?$filter=individualId%20eq%20'" + userID + "'&$format=json HTTP/1.1");
			sbRequest.AppendLine("GET getProgramObligationsSet?$format=json HTTP/1.1");
			sbRequest.AppendLine("\r");
			sbRequest.AppendLine("\r");
			sbRequest.AppendLine("--batch");
			sbRequest.AppendLine("Content-Type:application/http");
			sbRequest.AppendLine("Content-Transfer-Encoding: binary");
			sbRequest.AppendLine("\r");
			//sbRequest.AppendLine("GET tncDetailsModelsSet?$filter=individualId%20eq%20'" + userID + "'&$format=json HTTP/1.1");
			sbRequest.AppendLine("GET tncDetailsModelsSet?$format=json HTTP/1.1");
			sbRequest.AppendLine("\r");
			sbRequest.AppendLine("\r");
			sbRequest.AppendLine("--batch");
			sbRequest.AppendLine("Content-Type:application/http");
			sbRequest.AppendLine("Content-Transfer-Encoding: binary");
			sbRequest.AppendLine("\r");
			//sbRequest.AppendLine("GET PLTNCDetailsSet?$filter=individualId%20eq%20'" + userID + "'&$format=json HTTP/1.1");
			sbRequest.AppendLine("GET PLTNCDetailsSet?$format=json HTTP/1.1");
			sbRequest.AppendLine("\r");
			sbRequest.AppendLine("\r");
			sbRequest.AppendLine("--batch");
			sbRequest.AppendLine("Content-Type:application/http");
			sbRequest.AppendLine("Content-Transfer-Encoding: binary");
			sbRequest.AppendLine("\r");
			//sbRequest.AppendLine("GET GetSurveyByIndividualSet?$filter=individualId%20eq%20'" + userID + "'&$format=json HTTP/1.1");
			sbRequest.AppendLine("GET GetSurveyByIndividualSet?$format=json HTTP/1.1");
			sbRequest.AppendLine("\r");
			sbRequest.AppendLine("\r");
			sbRequest.AppendLine("--batch--");
